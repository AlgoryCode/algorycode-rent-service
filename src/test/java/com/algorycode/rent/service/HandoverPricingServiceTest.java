...
class HandoverPricingServiceTest {
  ...
  void quote_alToXK_route60_plus_legs() {
    long pId = 10L;
    long rId = 11L;
    when(handoverLocationRepository.findById(pId))
        .thenReturn(Optional.of(loc(pId, "AL", new BigDecimal("25"))));
    when(handoverLocationRepository.findById(rId))
        .thenReturn(Optional.of(loc(rId, "XK", new BigDecimal("0"))));
